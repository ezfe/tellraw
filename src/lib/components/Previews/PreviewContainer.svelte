...
{#if isBookPreview && !bookPreviewDisclaimerShown}
	<div class="row mb-2">
		<div class="col-8 offset-2">
			<p>
				Please understand that it is difficult for me to replicate the same text size and wrapping
				behavior as in regular Minecraft.
			</p>
			<p>
				You should verify that it looks correct from time-to-time in-game as well, as some text may
				not fit when it appears as though it will here!
			</p>
			<Button
				color="info"
				on:click={() => {
					bookPreviewDisclaimerShown.set(true);
				}}
			>
				<CheckCircle />
				OK
