...
export function previewGroupFromTranslate(
	...
	for (let i = 0; i < matches.length; i++) {
		...
		if (i > 0) {
			...
			middleSnippet.text = translationString.slice(
				priorMatch.index + priorMatch.length,
				match.index
			);
	...
	if (matches.length > 0) {
		...
		if (lastMatch.index + lastMatch.length < translationString.length) {
			...
			lastSnippet.text = translationString.slice(
				lastMatch.index + lastMatch.length,
				translationString.length
			);
